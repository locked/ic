...
class Server:
	...
	def get_maps(self):
		if self.maps==[]:
			maps = self.db.map.find({'status': 1})
			for m in maps:
				self.maps.append( m )
		return self.maps
	...
	def get_games(self):
		self.games = []
		games = self.db.game.find({'status': 1})
		for g in games:
			self.games.append( Game( g ) )
		return self.games

	def get_grounds(self, ground_id):
		self.grounds = []
		grounds = self.db.ground.find({'ground_id':ground_id})
		for g in grounds:
			self.grounds.append( g )
		return self.grounds

	def run(self):
		print "MAPS:", srv.get_maps()
		self.games = self.get_games()
		print "RUNNING GAMES ("+str(len(self.games))+"):"
		self.grs = {}
		for g in self.games:
			print "Start game [%s]" % str(g)
			gr = GameWorker(self,g)
			gr.daemon = True
			gr.start()
			#gr.rnd = -1
...
class GameApi():
	def __init__(self, gameworker, srv):
...
class GameWorker(multiprocessing.Process):
	def __init__(self, srv, game):
		...
		self.api = GameApi(self.game, self.srv)
		self.api.listen()
	...
	def load_ground(self):
		grd = self.srv.get_grounds(self.game.ground) #GroundData.objects.filter( ground=g.ground )
		ground = {}
		for g in grd:
